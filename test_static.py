...
if __name__ == "__main__":
    ...
    LDRBpth = os.path.join(dataroot, 'static')
    scenes_lists = gb.glob(os.path.join(LDRBpth, '*'))
    # print(dataroot)
    for scene_name in scenes_lists:
        scene_number = scene_name.split('static/')[-1]
        ...
        for i in tqdm(range(0,200,1)):
            ...
            interval = np.load(os.path.join(scene_name, 'master_slave_interval.npy'), allow_pickle=True)
            timestamps = []
            for j in range(len(interval.item()[int(i)]['old'])):
                timestamps.append(float(interval.item()[int(i)]['new'][j]))

            for time_num in range(len(timestamps)):
                ...
                fake_HDR_S, fake_HDR_Recon = model.inference(LDRB, event_leftB, event_rightB, B_all_start)
                ...
                ensure_dir(join(outputs_dir, 'static', scene_number, str(i).zfill(5)))
                cv2.imwrite(join(outputs_dir, 'static', scene_number, str(i).zfill(5), str('%.15f' % float(time_dynamic)) + '.hdr'), fake_HDR_S)
                ...
                # ensure_dir(join(outputs_dir.replace('ours/','ours_all/'), 'static', scene_number, str(i).zfill(5)))
                # cv2.imwrite(join(outputs_dir.replace('ours/','ours_all/'), 'static', scene_number, str(i).zfill(5), str('%.15f' % float(time_dynamic)) + '.hdr'), fake_HDR_Recon)